...
export default function AdminSettings() {
  ...
  return <section className={styles.wrapper} id="cadastros">
    ...
    {tab === "tables" && <div className={styles.layout}>{canManageConfig ? <form className={styles.formCard} onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const fd = new FormData(form); create("table", { number: fd.get("number"), label: fd.get("label"), seats: fd.get("seats") }, form); }}><span>Nova mesa</span><h3>Cadastrar mesa</h3><label>Número<input name="number" min="1" required type="number" /></label><label>Identificação opcional<input name="label" placeholder="Varanda, Salão..." /></label><label>Lugares<input name="seats" min="1" type="number" /></label><button type="submit">Criar mesa</button></form> : <div className={styles.formCard}><span>Mesas</span><h3>Somente consulta</h3><p>O administrador define a numeração das mesas e seus links permanentes de QR Code.</p></div>}
      <div className={styles.list}><div className={styles.listHeader}><strong>Mesas cadastradas</strong><span>{config.tables.length}</span></div>{config.tables.map((table) => <article className={styles.row} key={table.id}><div className={styles.number}>#{String(table.number).padStart(2, "0")}</div><div><strong>{table.label || `Mesa ${table.number}`}</strong><small>{table.seats ? `${table.seats} lugares · ` : ""}{table.status}</small><code>/m/{table.public_code}</code></div><div className={styles.rowActions}><button onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/m/${table.public_code}`)} type="button">Copiar link</button>{canManageConfig && <button className={table.active ? styles.danger : styles.successButton} onClick={() => patch("table", table.id, { active: !table.active })} type="button">{table.active ? "Desativar" : "Ativar"}</button>}</div></article>)}</div></div>}
