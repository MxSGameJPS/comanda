...
const actionLabels = { WAITER_LINKED: "Garçom vinculado", ORDER_CREATED: "Pedido criado", ITEM_REMOVED: "Item cancelado", ITEM_STATUS_CHANGED: "Status do item alterado", TABLE_CLOSED: "Comanda encerrada", TABLE_SESSION_CLOSED: "Comanda encerrada", PAYMENT_CREATED: "Pagamento registrado" };
const paymentLabels = { PIX: "PIX", CASH: "Dinheiro", CREDIT_CARD: "Crédito", DEBIT_CARD: "Débito", OTHER: "Outros" };
const receiptDate = new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" });

export default function AdminDashboard() {
  const [data, setData] = useState({ metrics: {}, paymentMethods: {}, employees: [], audit: [], receipts: [] });
  ...
  useEffect(() => {
    ...
    let unsubscribe = () => {};
    subscribeToBroadcast({ channel: realtimeTopic, event: "refresh", onChange: load, onStatus: (status) => { if (status === "connected") load(); } }).then((cleanup) => { unsubscribe = cleanup; });
    return () => unsubscribe();
  }, [load, realtimeTopic]);
  ...
  return <main className={styles.page}><aside className={styles.sidebar}><div className={styles.brand}><span>C</span><strong>Comanda</strong></div><nav><a className={styles.active} href="#resumo">Resumo</a><a href="#financeiro">Financeiro</a><a href="#comprovantes">Comprovantes</a><a href="#cadastros">Cadastros</a><a href="#funcionarios">Equipe</a><a href="#auditoria">Auditoria</a></nav><button onClick={logout} type="button">Sair</button><Link href="/">← Portal</Link></aside>
    <section className={styles.content}><header className={styles.header}><div><span className={styles.eyebrow}>Gestão</span><h1>Painel administrativo</h1><p>O administrador configura o restaurante e acompanha a operação.</p></div><div className={styles.date}>{today} · Hoje</div></header>
      ...
      <section className={styles.panel} id="comprovantes"><div className={styles.panelHeader}><div><span>Vendas encerradas</span><h2>Comprovantes internos</h2></div><span className={styles.counter}>{data.receipts?.length || 0}</span></div><div className={styles.receiptTable}><div className={styles.receiptHead}><span>Nº</span><span>Mesa</span><span>Cliente</span><span>Pagamento</span><span>Total</span><span>Fechamento</span><span></span></div>{data.receipts?.length ? data.receipts.map((item) => <div className={styles.receiptRow} key={item.id}><strong>#{String(item.receipt_number).padStart(6,"0")}</strong><span>Mesa {String(item.table_number).padStart(2,"0")}</span><span>{item.customer_name}</span><span>{paymentLabels[item.payment_snapshot?.[0]?.method] || "—"}</span><strong>{money.format(Number(item.total || 0))}</strong><span>{item.closed_at ? receiptDate.format(new Date(item.closed_at)) : "—"}</span><button onClick={() => openReceipt(item.id)} type="button">Visualizar</button></div>) : <p className={styles.emptyState}>{loading ? "Carregando comprovantes..." : "Nenhum comprovante interno gerado ainda."}</p>}</div></section>
    ...
    </section><SalesReceipt mode="admin" onClose={() => setReceipt(null)} receipt={receipt}/></main>;
}
