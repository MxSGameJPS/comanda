...
export default function CustomerMenu({ tableCode }) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.products}>{visibleProducts.map((product) => <article className={`${styles.productCard} ${product.image_url ? styles.withImage : ""}`} key={product.id}>
      {product.image_url && <div className={styles.productImage}><img alt={product.name} loading="lazy" src={product.image_url} /></div>}
      <div className={styles.productContent}><div className={styles.productTop}><div><span className={styles.station}>{product.station === "BAR" ? "Copa" : product.station === "KITCHEN" ? "Cozinha" : "Pedido"}</span><h2>{product.name}</h2><p>{product.description}</p></div><strong>{money.format(Number(product.price))}</strong></div>
      <div className={styles.productAction}><input aria-label={`Observação para ${product.name}`} onChange={(event) => setNotes((current) => ({ ...current, [product.id]: event.target.value }))} placeholder="Observação, se precisar" value={notes[product.id] || ""} /><button onClick={() => addProduct(product)} type="button">Adicionar</button></div></div>
    </article>)}</section>
