...
export async function POST(request) {
  try {
    ...
    await safeRealtimeBroadcast(`restaurant:${profile.restaurant_id}:operations`, "refresh", { type: "waiter_order_created" });
    return NextResponse.json({ order: row }, { status: 201 });
  } catch (error) {
    const message = String(error.message || "");
    if (message.includes("not linked")) {
      error.status = 403;
      error.message = "Você não está vinculado a esta mesa.";
    } else if (message.includes("not open")) {
      error.status = 409;
      error.message = "Esta comanda não aceita novos pedidos.";
    }
    return apiError(error, "Não foi possível adicionar o pedido à comanda.");
  }
}
