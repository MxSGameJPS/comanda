...
export async function realtimeBroadcast(topic, event = "refresh", payload = {}) {
  ...
  const encodedTopic = encodeURIComponent(String(topic));
  const encodedEvent = encodeURIComponent(String(event));
  const response = await fetch(`${url}/realtime/v1/api/broadcast/${encodedTopic}/events/${encodedEvent}`, {
    method: "POST",
    headers,
...
export async function safeRealtimeBroadcast(topic, event = "refresh", payload = {}) {
  try {
    return await realtimeBroadcast(topic, event, payload);
  } catch (error) {
    console.error("Supabase Realtime Broadcast falhou:", error?.message || error);
    return null;
  }
}

export async function storageEnsureBucket(bucket, { public: isPublic = true, fileSizeLimit = 5 * 1024 * 1024, allowedMimeTypes = ["image/jpeg", "image/png", "image/webp", "image/avif"] } = {}) {
  const { url, secretKey } = requireSecretKey();
  ...
  const existing = await fetch(`${url}/storage/v1/bucket/${encodedBucket}`, { headers, cache: "no-store" });
  if (existing.ok) return parseResponse(existing);
  if (existing.status !== 404) return parseResponse(existing);
  ...
  if (response.status === 409) return { id: bucket };
  return parseResponse(response);
}
