...
function rsGenerator(degree) {
  ...
  for (let i = 0; i < degree; i += 1) {
    generator = polynomialMultiply(generator, [1, root]);
...
function chooseVersion(byteLength) {
  for (let version = 1; version <= 5; version += 1) {
    const capacity = LEVEL_L[version].data * 8;
    const used = 4 + 8 + byteLength * 8;
    if (used <= capacity) return version;
  }
  throw new Error("URL longa demais para o QR Code da mesa.");
}

function encodeCodewords(text, version) {
  const bytes = [...new TextEncoder().encode(text)];
  const info = LEVEL_L[version];
  const bits = [];
  ...
  return [...data, ...rsRemainder(data, info.ecc)];
}
...
function formatBits(data) {
  ...
  while (31 - Math.clz32(value) >= generatorDegree) {
    value ^= generator << (31 - Math.clz32(value) - generatorDegree);
  }
...
function setFinder(matrix, row, col) {
  ...
  for (let dr = -1; dr <= 7; dr += 1) {
    for (let dc = -1; dc <= 7; dc += 1) {
      ...
      matrix[r][c] = Boolean(inside && (dr === 0 || dr === 6 || dc === 0 || dc === 6 || (dr >= 2 && dr <= 4 && dc >= 2 && dc <= 4)));
    }
  }
}
...
function setAlignment(matrix, centerRow, centerCol) {
  ...
  for (let dr = -2; dr <= 2; dr += 1) {
    for (let dc = -2; dc <= 2; dc += 1) {
      matrix[centerRow + dr][centerCol + dc] = Math.max(Math.abs(dr), Math.abs(dc)) !== 1;
    }
  }
}

function setupBase(version) {
  ...
  for (const row of LEVEL_L[version].align) {
    for (const col of LEVEL_L[version].align) setAlignment(matrix, row, col);
  }
  ...
  const format = formatBits(1 << 3); // error correction L + mask 0
  const getBit = (index) => ((format >>> index) & 1) !== 0;
...
export function makeQrMatrix(text) {
  ...
  const codewords = encodeCodewords(value, version);
  const bits = [];
  codewords.forEach((byte) => appendBits(bits, byte, 8));

  const matrix = setupBase(version);
  const size = matrix.length;
  let bitIndex = 0;
  let upward = true;
  ...
  for (let col = size - 1; col > 0; col -= 2) {
    ...
    for (let step = 0; step < size; step += 1) {
      ...
      for (let offset = 0; offset < 2; offset += 1) {
        ...
        matrix[row][currentCol] = Boolean(bit);
      }
    }
    upward = !upward;
  }
...
export function qrSvgString(text, scale = 8, border = 4) {
  const matrix = makeQrMatrix(text);
  const size = (matrix.length + border * 2) * scale;
  let path = "";
  matrix.forEach((row, r) => row.forEach((dark, c) => {
    if (dark) path += `M${(c + border) * scale} ${(r + border) * scale}h${scale}v${scale}h-${scale}z`;
  }));
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="white"/><path d="${path}" fill="black"/></svg>`;
}
