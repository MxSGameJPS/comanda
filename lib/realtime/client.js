...
export async function subscribeToBroadcast({ channel, event = "refresh", onChange, onStatus }) {
    ...
    try {
      ...
      socket.addEventListener("message", (message) => {
        ...
        if (packet.event === "phx_reply") {
          if (packet.payload?.status === "ok") {
            attempt = 0;
            onStatus?.("connected");
          } else if (packet.payload?.status === "error") {
            onStatus?.("error");
            socket?.close();
          }
          return;
        ...
        if (packet.event === "broadcast") {
          const envelope = packet.payload || {};
          if (!event || envelope.event === event) onChange?.(envelope.payload ?? envelope);
          return;
        }

        if (packet.event === "phx_error" || packet.event === "phx_close") onStatus?.("error");
      });
...
// Mantido para compatibilidade com código antigo. Novos painéis usam Broadcast,
// evitando expor JWTs de funcionários no navegador e evitando polling contínuo.
